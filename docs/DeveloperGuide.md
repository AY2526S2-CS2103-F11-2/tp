...
12. Enforce alphanumeric requirement in name validation
    Currently, the `add` and `edit` commands accept names that consist entirely of special characters such as ... or (((, since the name validation only requires at least  one character from the full allowed set (letters, numbers, and ( ) . - , ') rather than at least one letter or number. This means a contact can be saved with a name that contains no meaningful identifying information, which is unintended for a contact management application. A future enhancement would be to tighten the validation regex so that at least one alphanumeric character is required, while still allowing names like Prof. O'Brien or Li, Wei-Ming that legitimately combine letters with special characters.
